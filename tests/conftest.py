...
# Make the repo root importable so `import grag.*` resolves from tests/
sys.path.insert(0, str(Path(__file__).parent.parent))

# Patch tracing at module load time so cli.py's top-level tracing.init_tracing()
# call is a no-op when test modules import the package. Fixture-level patching fires
# too late (after test-module imports, which happen during pytest collection).
from grag import tracing as _tracing_mod  # noqa: E402
