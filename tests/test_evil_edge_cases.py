...
class TestExtractSourcesEvil:
    def test_nested_bracket_in_doc_name_rejected(self):
        # Regex [^\[\]]{1,200} forbids nested brackets, so a doc name containing
        # "[" breaks the marker before a closing "]" is reached -> no match.
        # Intentional: stops a truncated marker from greedily swallowing
        # following JSON content.
        from grag.engine import _extract_sources
...
class TestCitedPagesEvil:
    def test_reversed_range_only_first_number_matched_by_standalone(self):
        ...
        from grag.engine import _cited_pages
...
class TestCitedChunksEvil:
    def test_content_as_integer_raises_type_error(self):
        ...
        from grag.engine import _cited_chunks
...
class TestImageCandidatesEvil:
    def test_uppercase_hex_hash_not_matched_by_regex(self, monkeypatch):
        ...
        result = engine._image_candidates(json.dumps({"content": content}))
        assert result == []

    def test_null_content_field_skipped_safely(self, monkeypatch):
        from grag import engine, cli
        monkeypatch.setattr(engine, "_resolve_image_path", lambda h, c: "/fake/h.jpg")
        result = engine._image_candidates(json.dumps({"content": None}))
        assert result == []
...
class TestChatSessionEvil:
    def test_load_corrupted_json_raises_decode_error(self, tmp_path):
        ...
        sess = engine.ChatSession("/wdir", "openai", "oneshot", sp)
        with pytest.raises(json.JSONDecodeError):
            sess.load()
    ...
    def test_none_provider_coerced_to_default_at_init(self, tmp_path, monkeypatch):
        ...
        sess = engine.ChatSession("/wdir", None, "oneshot", sp)
...
class TestParseArgsEvil:
    ...
    def test_agent_flag_combined_with_other_flags_all_parsed(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(cli, "_SESSIONS_DIR", tmp_path)
        _, provider, rerank_mode, _, agent_mode = cli._parse_args()
    ...
    def test_resume_followed_by_dash_flag_is_bare_resume(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(cli, "_SESSIONS_DIR", tmp_path)
        _, _, _, session_path, _ = cli._parse_args()
