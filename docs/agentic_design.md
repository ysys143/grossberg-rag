...
모든 미결 항목 해소. 구현 파일: `kb_tool.py`, `agent.py`.

| 항목 | 결정 | 근거 |
|---|---|---|
| 키워드 분리 | **`concepts`(hl) / `entities`(ll) 2분리** | 라우팅이 tool 인자에 녹음, 별도 분류기 불필요 |
| tool 반환값 | **컨텍스트만** (`only_need_context=True` 고정) | tool은 순수 retriever. agent(gpt-5.5)가 합성·인용 처리 — double-LLM 비용 없음 |
| SDK 의존성 | **zero** — httpx 직접 호출 | 기존 llm.py 패턴과 일관성. LangChain 미도입 |
| 웹 백엔드 | **OpenAI built-in `web_search_preview`** (Phase 1) | 추가 패키지/API 키 불필요. Phase 2 TODO: Tavily 함수 tool로 교체 → 요약 미들웨어 적용 가능 |
| router 처리 | **(A) 프리필터 유지** | 기존 `router.py` 그대로. agent 진입 전 싸게 거름 |
